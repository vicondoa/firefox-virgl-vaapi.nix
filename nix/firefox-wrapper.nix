...
stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p "$out/lib/firefox-virgl-vaapi"
    original="$out/lib/firefox-virgl-vaapi/$main_name-original"
    mv "$main" "$original"
    makeWrapper "$original" "$main" ${makeWrapperArgs}
    rm -f "$out/bin/.$main_name-wrapped" "$out/bin/$main_name.virgl-vaapi-original"
